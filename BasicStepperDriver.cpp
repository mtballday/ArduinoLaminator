...
void BasicStepperDriver::startConstantMove(bool positive_direction){
    float speed;
    // set up new move
    dir_state = positive_direction;
    last_action_end = 0;
    steps_remaining = 10000;
    step_count = 0;
    rest = 0;
    profile.constant_motion = true;
...
void BasicStepperDriver::startConstantRotate(bool forward){
    startConstantMove(forward);
}
...
void BasicStepperDriver::calcStepPulse(void){
    ...
    if(!(profile.constant_motion == true && getCurrentState() == CRUISING)){
      steps_remaining--;
      step_count++;
    }
